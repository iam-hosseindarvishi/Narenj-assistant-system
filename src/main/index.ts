...
function registerIpcHandlers(manager: DatabaseManager): void {
  ...
  const deduped = manual.cleanupBrokenManualLinks()
  if (cleaned > 0 || deduped > 0) console.log(`Cleaned ${cleaned} orphaned suggested links, ${deduped} broken/duplicate manual links`)
