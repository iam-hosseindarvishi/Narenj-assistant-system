  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-bl from-slate-900 to-slate-700 p-4">
    <div class="w-full max-w-sm bg-white rounded-2xl shadow-xl p-8">
      <div class="text-center mb-8">
        <div class="text-2xl font-bold text-slate-800">پنل مدیریت نارنج</div>
        <div class="text-sm text-slate-500 mt-1">ورود به سامانه</div>
      </div>
      <form @submit.prevent="submit" class="space-y-4">
        <div>
          <label class="block text-sm text-slate-600 mb-1">نام کاربری</label>
          <input v-model="username" class="input" autocomplete="username" required />
        </div>
        <div>
          <label class="block text-sm text-slate-600 mb-1">گذرواژه</label>
