<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <div class="text-sm text-slate-500">تطبیق کارمزدهای بانک با حسابداری و تجمیع روزانه</div>
    ...
    <div class="space-y-3">
      <div v-for="day in rows" :key="day.dateJalali" class="card">
        <div class="flex items-center justify-between mb-3">
          <div>
            <span class="font-mono font-semibold text-slate-800">{{ day.dateJalali }}</span>
            <span class="text-sm text-slate-500 mr-3">
              جمع روز: {{ money(day.totalAmount) }}
            </span>
          </div>
          <div class="flex items-center gap-2">
            <span v-if="day.registered" class="badge bg-emerald-100 text-emerald-700">ثبت‌شده</span>
            <button
